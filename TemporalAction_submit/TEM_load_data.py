...
def getFullData(dataSet):
    ...
    batch_anchor_xmax=[]
    batch_anchor_feature=[]
    for i in list(range(len(video_list))):
        if i%100==0:
        ...
        for j in list(range(len(video_labels))):
        ...
        tmp_anchor_xmin=[tgap*i for i in list(range(tscale))]
        tmp_anchor_xmax=[tgap*i for i in list(range(1,tscale+1))]
    ...
    for idx in list(range(num_data)):
        ...
        match_score_action=[]
        for jdx in list(range(len(anchor_xmin))):
